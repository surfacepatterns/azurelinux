...
version_release_matching_groups = [
    frozenset(
        [
            ...
            "SPECS-SIGNED/kernel-uki/kernel-uki-signed.spec",
    ...
    frozenset(["SPECS/jflex/jflex.spec", "SPECS/jflex/jflex-bootstrap.spec"]),
    frozenset(
        [
...
version_matching_groups = [
    frozenset(["SPECS/hyperv-daemons/hyperv-daemons.spec", "SPECS/kernel/kernel.spec"]),
    frozenset(
        [
    ...
    frozenset(["SPECS/influxdb/influxdb.spec", "SPECS/influx-cli/influx-cli.spec"]),
    frozenset(
        [
...
mstflintver_matching_groups = [
    frozenset(["SPECS/mstflint/mstflint.spec", "SPECS/kernel/kernel.spec"])
]


def check_spec_tags(
    base_path: str, tags: List[str], groups: List[FrozenSet]
) -> Set[FrozenSet]:
...
def check_version_release_match_groups(base_path: str) -> Set[FrozenSet]:
    return check_spec_tags(
        base_path, ["epoch", "version", "release"], version_release_matching_groups
    )
...
def check_matches(base_path: str):
    ...
    if (
        len(version_match_errors)
        or len(version_release_match_errors)
        or len(sdkver_match_errors)
        or len(mstflintver_match_errors)
    ):
